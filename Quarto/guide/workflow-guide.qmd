...
| Multi-agent workflows | 11 specialized agents for proofreading, layout, pedagogy, code review, literature search |
...
This workflow was developed over 6+ sessions building a PhD course on Causal Panel Data. The result: 6 complete lectures (140+ slides each), with Beamer + Quarto versions, interactive Plotly charts, TikZ diagrams, and Python replication scripts --- all managed by the orchestrator and reviewed by 11 specialized agents across 5 quality dimensions.
...
Claude automatically:
  ...
  → Runs /devils-advocate (challenge assumptions, missing alternatives)
...
You: "Analyze this dataset and produce publication-ready output"
     ↓
Claude automatically:
  → Runs /data-analysis-python (explore, model, tables, figures)
  → Runs /review-python (code quality, reproducibility)
  → Runs /econ-visualization if charts needed (publication-quality figures)
  → Produces Python script, tables, and figures
  → Scores against quality gates
     ↓
...
```
You: "Search for papers on AI coaching effectiveness and synthesize the key findings"
     ↓
Claude automatically:
  → Runs /lit-search (CrossRef, Semantic Scholar, Tavily)
  → Runs /lit-read (structured reading notes per paper)
  → Runs /lit-review (synthesize themes, identify gaps)
  → Runs /validate-bib (verify all citations are in bibliography)
  → Produces paper list, reading notes, and synthesis report
     ↓
You see: "Found 23 papers. 15 kept after screening. 4 major themes identified."
```
...
Before you can use this workflow, you need VS Code and Claude Code on your machine. Three steps:

### Step 1: Install VS Code

Download from [code.visualstudio.com](https://code.visualstudio.com) (free, available for macOS, Windows, and Linux).
...
Claude Code runs as both a **CLI tool** and a **VS Code extension** --- same capabilities, same configuration, different interface. Install whichever you prefer (or both).
...
Requires **Node.js 18+**. Check with `node --version`. If missing, get it from [nodejs.org](https://nodejs.org).
...
Once installed, open the Claude Code panel from the sidebar (click the Claude icon) or press `Cmd+Shift+P` → "Claude Code: Open".

:::

### Step 3: Authenticate

Run `claude` in any terminal. Your browser will open for authentication.

**Subscription options:**

| Option | How It Works | Best For |
|--------|-------------|----------|
| **Claude Pro / Max** | Monthly subscription includes Claude Code usage | Most academics --- generous limits |
| **API key** | Pay per token via [Anthropic Console](https://console.anthropic.com) | Heavy users or CI/CD |

To use an API key instead of browser login, set the environment variable:
...
## Using a Third-Party API Provider (GLM, OpenRouter, etc.)

If you are in China or prefer a non-Anthropic provider, configure Claude Code to use an Anthropic-compatible API by editing your **user-level settings file**. The same JSON config works on both platforms --- only the file path differs.
...
Paste the following (example: Zhipu GLM):
...
| Field | Purpose |
|-------|---------|
| `ANTHROPIC_AUTH_TOKEN` | Your API key from the provider |
| `ANTHROPIC_BASE_URL` | The provider's Anthropic-compatible endpoint |
| `ANTHROPIC_DEFAULT_*_MODEL` | Model ID for each Claude Code tier (haiku/sonnet/opus) |
| `model` | Default model Claude Code uses |
...
| AWS Bedrock | Set `CLAUDE_CODE_USE_BEDROCK=1` instead |
| Google Vertex | Set `CLAUDE_CODE_USE_VERTEX=1` instead |

> **Tip:** You can also place this config in the **project-level** `.claude/settings.json` if you want different providers per project.
:::

You can also authenticate inside VS Code --- the extension will prompt you on first use.
...
| Python | Figures & data analysis | [python.org](https://www.python.org/) |
...
I am starting to work on **[PROJECT NAME]** in this repo. **[Describe your project in 2--3 sentences --- what you're building, who it's for, what tools you use (e.g., LaTeX/Beamer, Python, Quarto).]**
...
└── knowledge-base.md            # domain-specific knowledge (customize for your field)
```
...
## Reproducibility
- np.random.seed() called ONCE at top (YYYYMMDD format)
...
The template includes examples for LaTeX, Python, Jupyter, and multi-language workflows.
...
| `/translate-to-quarto` | Beamer to Quarto translation | Translating existing lectures |
| `/qa-quarto` | Adversarial Quarto QA | After translating Beamer to Quarto |
| `/extract-tikz` | TikZ to SVG conversion | Working with diagrams |
...
| `/visual-audit` | Adversarial layout audit | Before presentations |
| `/pedagogy-review` | Narrative and pedagogy review | Reviewing teaching materials |
| `/review-python` | Python code quality review | After writing Python scripts |
| `/review-paper` | Full manuscript review | Before submission or after a draft |
| `/devils-advocate` | Design challenge questions | Before building on a foundation |

### Research & Literature

| Skill | Purpose | When to Use |
|-------|---------|------------|
| `/lit-search` | Academic literature search (CrossRef, Semantic Scholar, Tavily) | Finding papers on a topic |
| `/lit-read` | Structured reading of academic PDFs | Processing a batch of papers |
| `/lit-review` | Literature synthesis and gap identification | Starting a new project or section |
| `/research-ideation` | Research questions and strategies | Brainstorming phase |
| `/interview-me` | Interactive research interview | When you have an intuition but not a plan |

### Data Analysis

| Skill | Purpose | When to Use |
|-------|---------|------------|
| `/data-analysis-python` | End-to-end Python analysis workflow | Empirical analysis phase |
| `/python-regression` | Python regression with publication tables | Running regressions in Python |
| `/stata-regression` | Stata regression with publication tables | Running regressions in Stata |
| `/econ-visualization` | Publication-quality charts and graphs | Creating figures for papers |
...
| `/validate-bib` | Bibliography validation | Before compilation |
| `/learn` | Extract discoveries into skills | After noticing repeated patterns |
| `/context-status` | Show session health and context usage | Checking context window state |
| `/deep-audit` | Repository-wide consistency audit | After broad infrastructure changes |
| `/jupyter-notebook` | Create structured Jupyter notebooks | Building experiments or tutorials |
...
The `/codex` skill provides integration with OpenAI's Codex CLI for batch operations. If you use both Claude Code and Codex, this skill enables the synergy pattern: Claude leads strategy and analysis, Codex executes batch operations. This is an optional integration --- the workflow works fully without it.
:::
...
+-- literature-search.md  # Academic literature search and screening
```
...
## Corrections Log
- [LEARN:python-code] Package X drops obs silently when covariate is missing
...
Phase 2: Translate (e.g., Stata -> Python)
...
In one course, we discovered that a widely-used Python package silently produced **incorrect estimates** due to a subtle specification issue. This bug was caught 3 times in different scripts. Without the replication-first protocol, these wrong numbers would have been taught to PhD students.
:::
...
- [LEARN:citation] Post-LASSO is Belloni (2013), NOT Belloni (2014)
- [LEARN:python-code] Package X: ALWAYS include intercept in design matrix
- [LEARN:workflow] Every Beamer edit must auto-sync to Quarto
```
...
Ten skills support the research workflow beyond slide development:

| Skill | What It Does | When to Use |
|-------|-------------|-------------|
| `/lit-search [query]` | Academic literature search via CrossRef, Semantic Scholar, and Tavily APIs | Finding papers on a topic |
| `/lit-read [directory]` | Structured reading of academic PDFs in 3 phases (cluster, read, abstract) | Processing a batch of papers |
| `/lit-review [topic]` | Synthesize and identify gaps in the literature | Starting a new project or section |
...
| `/data-analysis-python [data]` | End-to-end Python analysis: explore, regress, produce publication-ready output | Empirical analysis phase |
| `/python-regression` | Python regression with publication-ready output tables | Running regressions in Python |
| `/stata-regression` | Stata regression with publication-ready output tables | Running regressions in Stata |
| `/econ-visualization` | Publication-quality charts and graphs for economics papers | Creating figures for papers |

These skills produce structured reports saved to `quality_reports/`. The `/data-analysis-python` skill also generates Python scripts (saved to `scripts/python/`) and runs the python-reviewer agent automatically.
...
```
/research-ideation ─────┐
                        ├──→ /lit-search ──→ /lit-read ──→ /lit-review ──→ /data-analysis-python ──→ /review-paper
/interview-me ──────────┘                                                     ↑                  |
                                                                      /python-regression         |
                                                                      /stata-regression          |
                                                                      /econ-visualization        |
...
**Enter mid-pipeline.** You do not have to start from ideation. If you already have data, start with `/data-analysis-python` and work backwards to the research question. If you already have a draft, start with `/review-paper`. The skills are modular --- use what you need, skip what you don't.
...
- [ ] Setup script installs all dependencies (`requirements.txt`, `pyproject.toml`, etc.)
...
├── code/
│   ├── 00_setup.py          # Install dependencies
...
- prepare_data.py (data generation is frozen)
...
8. **Progressive disclosure.** Start with CLAUDE.md + 2--3 rules. Add more as your workflow matures. Newcomers should not face 19 rules on day one.
...
| Literature Search | `.claude/agents/literature-search.md` | Academic literature search and screening |
...
| `/lit-search` | `.claude/skills/lit-search/` | Academic literature search (CrossRef, Semantic Scholar, Tavily) |
| `/lit-read` | `.claude/skills/lit-read/` | Structured reading of academic PDFs |
...
| `/data-analysis-python` | `.claude/skills/data-analysis-python/` | End-to-end Python analysis |
| `/python-regression` | `.claude/skills/python-regression/` | Python regression with publication tables |
| `/stata-regression` | `.claude/skills/stata-regression/` | Stata regression with publication tables |
| `/econ-visualization` | `.claude/skills/econ-visualization/` | Publication-quality charts and graphs |
| `/jupyter-notebook` | `.claude/skills/jupyter-notebook/` | Create structured Jupyter notebooks |
...
| Knowledge Base | `knowledge-base.md` | Project-specific domain knowledge |
...
| Knowledge Base Template | `knowledge-base-template.md` | `.tex`, `.qmd`, `*.py` |
...
|------|------|--------------|
| Literature search redirect | PreToolUse[WebSearch\|WebFetch] (command) | `.claude/hooks/literature-search.py` |
