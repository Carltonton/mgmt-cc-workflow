...
Skills like `/proofread` can be invoked two ways:
...
**You talk, Claude orchestrates.** The 11 agents, 26 skills, and 19 rules exist so you don't have to think about them. Describe your goal, approve the plan, and let the system work.
:::

::: {.callout-note}
## You Don't Need All of This on Day One

This guide describes the full system --- 11 agents, 26 skills, 19 rules. That is the ceiling, not the floor. **Start with just CLAUDE.md and 2--3 skills** (`/proofread`, `/commit`). Add rules and agents as you discover what you need. The template is designed for progressive adoption: fork it, fill in the placeholders, and start working. Everything else is there when you're ready.
:::
...
---
name: deploy
description: Render Quarto slides and sync to docs/
argument-hint: "[Lecture name or 'all']"
---

# Steps:
1. Run quarto render
2. Copy output to docs/
...
| `/research-ideation` | Research topics from empirical observations (PXYV framework) | From phenomenon to research question |
...
| Skill | Purpose | When to Use |
|-------|---------|------------|
| `/data-analysis-python` | End-to-end Python analysis workflow (EDA, regression, tables, figures) | Empirical analysis phase |
...
| `skills` | Make specific skills available | `["deploy"]` |
...
| `/research-ideation [topic]` | Generate research topics from empirical observations using PXYV framework | From phenomenon to research question |
...
| Field | Purpose | Example |
|-------|---------|---------|
| `name` | Display name in `/` menu | `deploy` |
...
|--------|-----------|-------------|
| `$ARGUMENTS` | Full argument string after `/skill-name` | `/deploy Lecture01` → `$ARGUMENTS` = `Lecture01` |
...
| `/research-ideation` | `.claude/skills/research-ideation/` | Research topics from empirical observations (PXYV framework) |
...
| `/data-analysis-python` | `.claude/skills/data-analysis-python/` | End-to-end Python analysis (EDA, regression, tables, figures) |
...
2. Ensure `TEXINPUTS` includes `Preambles/`: set when compiling manually
