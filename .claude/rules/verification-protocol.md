...
## For Python Scripts:
1. Run with `python scripts/python/scriptname.py` or `jupyter nbconvert --execute notebook.ipynb`
2. Verify output files (PNG, CSV, Parquet, PKL) were created with non-zero size
3. Check for import errors or missing dependencies
4. Spot-check numerical results for reasonable magnitude
5. Verify figures saved with correct DPI and transparent background
6. Check logs for warnings or exceptions
...
## For LaTeX Research Papers (paper/):
1. Compile with xelatex (3-pass) and bibtex: `cd paper && xelatex main && bibtex main && xelatex main && xelatex main`
2. Check for compilation errors and warnings
3. Open PDF to verify figures render correctly
4. Verify bibliography citations resolve
5. Check for overfull hbox warnings in log file
...
- **Python seeds not set**: Non-reproducible results → always verify seeds are set
- **Missing virtual environment**: Dependency conflicts → activate `.venv` before running
- **Hardcoded paths**: Code breaks on other machines → use `pathlib.Path` with relative paths
